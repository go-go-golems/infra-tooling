...
This feature is exclusively available with [GoReleaser Pro](https://goreleaser.com/pro/).
...
You can also specify `GOARCH` and `GGOARCH`, which only take effect if you set `partial.by` to `target`.
...
You can find an example project [here](https://github.com/goreleaser/example-split-merge-real). Feel free to dive into the workflow and the GoReleaser configuration.
