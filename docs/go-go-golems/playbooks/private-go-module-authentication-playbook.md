...
```json
{
  ...
  "bound_claims": {
    ...
    "repository": "hyperslop-systems/<repo>"
  },
...
The policy is missing, or exists in git but was never applied. `vault policy read
gha-<repo>-private-dependencies` distinguishes the two.
...
- [../../platform/source-repo-to-gitops-pr.md](../../platform/source-repo-to-gitops-pr.md) —
  the caller contract for the shared publish workflow, including
  `private_dependencies_profile`.
